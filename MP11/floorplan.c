...
int is_leaf_node(node_t* ptr) {
  ...
  if (ptr->left == NULL && ptr->right == NULL) // leaf in no child exists
    return 1;
  else
    return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if (ptr->left != NULL || ptr->right != NULL) // internal if a child exists
    return 1;
  else
    return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  int lef = 0, righ = 0;
  if (a == b) // base case when found
    return 1;
  else if (a->left == NULL && a->right == NULL) // base case when not found (reached bottom of tree)
    return 0;
  if (a->left != NULL)
    lef = is_in_subtree(a->left, b); // left recursive call
  if (a->right != NULL)
    righ = is_in_subtree(a->right, b); // right recursive calloc
  if (lef == 0 && righ == 0) // if not found down left and right branches
    return 0;
  else
    return 1;
}
...
void rotate(node_t* ptr) {
  ...
  int temp = ptr->module->w; // width stored in temp
  ptr->module->w = ptr->module->h; // assign height to width
  ptr->module->h = temp; // assign temp (width) to height
  return;
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if (ptr->cutline == V) // switch V to H
    ptr->cutline = H;
  else // switch H to V
    ptr->cutline = V;
  return;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  int side1, side2; // left is 1, right is 0
  if (a->parent->left == a)
    side1 = 1;
  else
    side1 = 0;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  if (ptr->left != NULL)
    postfix_traversal(ptr->left, nth, expression); // left recursive call
  if (ptr->right != NULL)
    postfix_traversal(ptr->right, nth, expression); // right recursive call
  if (ptr->module == NULL) // means node is cutline
    expression[*nth].cutline = ptr->cutline;
  else // means node is module
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO:
  // If there is only one node left to go, that is the base case.
  // Create the node and then return it
  if(n == num_modules-1)
  {
    node_t *return_node = (node_t *)malloc(sizeof(node_t));
    return_node->parent = par;
    return_node->left = NULL;
    return_node->right = NULL;
    return_node->cutline = UNDEFINED_CUTLINE;
    return_node->module = &modules[n];
    return return_node;
  }
  // Otherwise have the recursive case
  node_t *return_node = (node_t *)malloc(sizeof(node_t));
  return_node->parent = par;
  return_node->cutline = V;
  return_node->module = NULL;

  // Create the module that is on the right side
  node_t *node_mod = (node_t *)malloc(sizeof(node_t));
  node_mod->parent = return_node;
  node_mod->left = NULL;
  node_mod->right = NULL;
  node_mod->cutline = UNDEFINED_CUTLINE;
  node_mod->module = &modules[n];

  // Assign the module, and recurse to the next level, then return the created node
  return_node->right = node_mod;
  return_node->left = init_slicing_tree(return_node, n+1);
  return return_node;
