...
NODE* lowest_common_ancestor (NODE* root , int first_number , int second_number )
{
	/* your code goes here */
	int cur = root->d;
	if ( (cur >= first_number && cur <= second_number) || (cur <= first_number && cur >= second_number) )
		return root;
	else if ( first_number <= cur && second_number <= cur )
		lowest_common_ancestor( root->left, first_number, second_number );
	else // else if (first_number >= cur && second_number >= cur)
		lowest_common_ancestor( root->right, first_number, second_number );
}
